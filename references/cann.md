...
2. **核实静态候选**：扫描器只收集具有接口归属依据的候选，包括明确的 CANN 前缀、限定命名空间、Python 导入、动态绑定、已解析接收者类型，以及 `using namespace AscendC` 下的未限定名称。仅因文件包含 CANN 头文件、经本仓头文件传播上下文，或名称看起来像函数/常量的普通 token 不收集、不整理、不统计。
   ...
   自动结果仍不代表完成审计。阅读伴随的 `.scan.json`、`.catalog.json`、`.review.json` 和覆盖信息。参照 [判定规则](method.md)，核实 CANN 归属、仓库本地定义、宏拼接、动态符号、导入别名及 Ascend C 方法接收者类型。前缀或 include 上下文本身不是确认归属的充分证据；明确调用证据与最新官方正文名称同时命中且无本仓同名定义时，可以自动归类“已匹配”。
...
3. **核实文档和缺口**：官方目录用于定位页面，最终匹配以已获取的 API 正文为证据，区分同名不同命名空间或类。只检查名称，不推断参数、返回值或运行时兼容性。复核 `.catalog.json` 中每章选中/未选中计数；新版本目录结构变化、空目录、外链、抓取失败都需处理。
   ...
   未匹配项必须在上述四类最新官方 API 文档内再次检索并打开结果核实。搜索无结果不是未公开的证明；其他章节、旧版文档、开源实现、搜索摘要不能代替当前版本公开 API 文档。若四类目录遗漏了其中所属的 API 页面，在 `.extra-pages.json` 中补充当前版本 URL 后带 `--extra-pages` 重跑。参考 [文档获取说明](method.md#官方文档获取) 排查网站变化。

4. **保存复核并生成最终报告**：按 [review 格式](method.md#人工复核文件) 更新自动生成的 `.review.json`，注明依据；核实扫描范围及 API 目录覆盖后才标记相应复核开关。
...
本文件中的 SKILL_DIR 仍指 skill 根目录。通用流程以 ../SKILL.md 为准。
