...
文件级 include 上下文不能把局部变量、模板参数、头文件保护宏或普通函数自动提升为接口候选。这些普通 token 不收集、不整理，也不写入报告或 `.scan.json`。人工发现扫描遗漏的真实接口时，通过 review 补充。
...
脚本先按官方目录一级分类限定范围，只接受“API参考、算子库、通信库、加速库”。“API参考”下的正文页全部纳入；其余三类只纳入标题、上级目录或路径明确标注 API、接口参考、数据结构、枚举、宏或常量的页面。“编程语言、调试与分析工具、参考/基础数据结构和接口、更多/应用开发、TBE 与 AI CPU 算子开发、LLM DataDist、DataFlow、ISP”等其他分类一律排除，即使页面标题或路径含 `API` 也不能作为名称对齐证据。

目录筛选仍需 agent 核对；`catalog_reviewed` 只有在检查四类目录的遗漏、外链、未展开节点和版本后才置为 true。API 在新版本被移出这四类时，将其记录为范围变化或文档缺口，不能自行扩大证据范围，也不能无依据沿用旧版。脚本只接受同一版本 hiascend 文档页；人工补充页面也必须属于这四类。
...
将 `VERSION/PATH` 替换为实际已验证页面路径。通过 `--extra-pages FILE.json` 添加；补充前必须确认页面属于上述四类，不用其他章节或未经验证的通用域名搜索结果充当覆盖证明。
