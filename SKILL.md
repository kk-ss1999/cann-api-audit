...
1. **扫描并构建文档证据**：
   ...
   默认解析官方 `latest`，获取整个版本的章节目录，仅从“API参考、算子库、通信库、加速库”四类中选取公开 API 页面并下载正文。“编程语言、调试与分析工具、参考/基础数据结构和接口、更多/应用开发、TBE 与 AI CPU 算子开发、LLM DataDist、DataFlow、ISP”等章节不作为对齐证据，即使标题或路径含 `API` 也排除。首次可能较慢，脚本定期输出进度，缓存按实际版本隔离；同一命令可续跑，`--refresh` 强制刷新。

2. **核实静态候选**：扫描器只收集具有接口归属依据的候选，包括明确的 CANN 前缀、限定命名空间、Python 导入、动态绑定、已解析接收者类型，以及 `using namespace AscendC` 下的未限定名称。仅因文件包含 CANN 头文件、经本仓头文件传播上下文，或名称看起来像函数/常量的普通 token 不收集、不整理、不统计。
...
3. **核实文档和缺口**：官方目录用于定位页面，最终匹配以已获取的 API 正文为证据，区分同名不同命名空间或类。只检查名称，不推断参数、返回值或运行时兼容性。复核 `.catalog.json` 中每章选中/未选中计数；新版本目录结构变化、空目录、外链、抓取失败都需处理。
   ...
   未匹配项必须在上述四类最新官方 API 文档内再次检索并打开结果核实。搜索无结果不是未公开的证明；其他章节、旧版文档、开源实现、搜索摘要不能代替当前版本公开 API 文档。若四类目录遗漏了其中所属的 API 页面，在 `.extra-pages.json` 中补充当前版本 URL 后带 `--extra-pages` 重跑。参考 [文档获取说明](references/method.md#官方文档获取) 排查网站变化。
...
4. **保存复核并生成最终报告**：按 [review 格式](references/method.md#人工复核文件) 更新自动生成的 `.review.json`，注明依据；核实扫描范围及 API 目录覆盖后才标记相应复核开关。
   ...
   报告只列出“官方文档未找到”“待核实”和“已匹配”的 CANN 依赖，保留这些接口的全部代码出现位置。经复核属于纯本地实现、其他组件或仅注释/说明文字的名称，只记录在 `.review.json`，不进入 Markdown 报告。报告不展示内部识别线索。不得把待核实包装为通过。文档/扫描有缺口时，保留阶段性结果并说明具体缺口及后续动作。

## 输出和边界

- 用户交付为 Markdown；同名 JSON 文件仅用于缓存、复核和复现，无须要求用户维护它们。
- 报告记录仓库/commit/工作区状态、扫描时间、实际 CANN 版本、官方入口、覆盖率、接口名称/类别、判定、代码路径/行号、匹配证据 URL；不输出扫描器内部识别线索或已排除的本地名称。
