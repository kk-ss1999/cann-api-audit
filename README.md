...
支持本地目录和远程 Git URL，不限于 vLLM Ascend。
...
| 文档范围 | 最新 CANN 官方文档的 API 章节，包括通信库、加速库等位于 `/API/` 之外的章节 |
...
| **非 CANN / 本地定义** | 经复核属于仓库自定义接口或其他组件，不作为 CANN 未公开接口报告 |

每个接口附代码文件、行号、代码片段及识别依据；匹配项附官方文档链接。报告也记录仓库 commit、工作区状态、实际 CANN 版本、扫描时间和文档覆盖情况。
...
首次运行需要获取官方文档目录和所选 API 正文，可能涉及上万页面；源码规模也会影响候选数量和报告大小。脚本会输出扫描和下载进度。
